	...
	<div class="right_div">
		...
		<table class="table table-hover admin_table">
			...
			<?php
				...
				$img="SELECT * FROM imagesubmenu ORDER BY id DESC";
...
<?php
	require_once '../pages/footer.php';
		if(isset($_POST['submit'])){
			...
			if(empty($name)){
				header("Location:imgsubmenu.php?report=0&action=insert&message=");

			}
			else{
			$mysql = "INSERT INTO imagesubmenu (category) VALUES ('$name')";
					if($sqlrun = mysqli_query($db,$mysql)){
						header("Location:imgsubmenu.php?report=1&action=insert&message=");
					}
					else{
						header("Location:imgsubmenu.php?report=0&action=insert&message='Insert Fail'");
