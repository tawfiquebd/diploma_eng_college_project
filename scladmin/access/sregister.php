...
<div class="col-lg-8 col-sm-8 col-md-8 col-xs-12">
	<div class="right_div">
		<form action="" enctype="multipart/form-data" method="post">
		    ...
		    <div class="form-group col-lg-3 col-md-3 col-sm-3 col-xs-12">
			    <select class="form-control about_select" name='class'>
			    	<?php 
			    		$list="SELECT * FROM classsubmenu ";
	...
	require_once '../pages/footer.php';
		if(isset($_POST['sregistration'])){
			...
			if(empty($stdid) || empty($class) || empty($sec) || empty($croll) || empty($sname) || empty($fname) || empty($mname) || empty($blood) || empty($nationality) || empty($gphone) || empty($age) || empty($hcond) || empty($religion) || empty($day) || empty($month) || empty($year) || empty($present) || empty($parmanent) || empty($image)){
						header("Location:sregister.php?report=0&action=empty&message=''");
			}

			else{
				 $uselect="SELECT * FROM student WHERE std_id ='$stdid' AND clsroll='$croll'";
					    if($found=mysql_query($uselect)){
					      if(mysql_num_rows($found)>1){
					          $data= mysql_fetch_assoc($found);
								header("Location:sregister.php?report=0&action=found&message=''");
					      }else{
					        	echo "What the hell ?";
					    }
			}/* else if end here */
			else{
				header("Location:sregister.php?report=0&action=mediaimage&message=''");
