	...
	<div class="right_div">
		...
		<table class="table table-hover admin_table">
			...
			<?php
				...
				$img="SELECT * FROM videosubmenu ORDER BY id DESC";
				if($imgfound=mysqli_query($db,$img)){
					while($imginfo=mysqli_fetch_assoc($imgfound)){
						...
						echo '<tr>
								...
								<td>'.$category.'</td>
								<td>
									<a href="'.$current_page.'?post_id='.$id.'&action=delete&from=submenu&media=" title="Completely Delete">
...
<?php
	require_once '../pages/footer.php';
		if(isset($_POST['submit'])){
			...
			if(empty($name)){
				header("Location:videosubmenu.php?report=0&action=insert&message=");

			}
			else{
			$mysql = "INSERT INTO videosubmenu (category) VALUES ('$name')";
					if($sqlrun = mysqli_query($db,$mysql)){
						header("Location:videosubmenu.php?report=1&action=insert&message=");
					}
					else{
						header("Location:videosubmenu.php?report=0&action=insert&message='Insert Fail'");
