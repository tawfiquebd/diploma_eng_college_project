...
<div class="col-lg-8 col-sm-8 col-md-8 col-xs-12">
	<div class="right_div">
		...
		<table class="table table-hover admin_table">
			...
			<tbody>
			<?php
				$img="SELECT * FROM classsubmenu ORDER BY id DESC";
...
<?php
	require_once '../pages/footer.php';
		if(isset($_POST['submit'])){
			...
			if(empty($name)){
				header("Location:clssubmenu.php?report=0&action=insert&message=");

			}
			else{
			$mysql = "INSERT INTO classsubmenu (category) VALUES ('$name')";
					if($sqlrun = mysqli_query($db,$mysql)){
						header("Location:clssubmenu.php?report=1&action=insert&message=");
					}
					else{
						header("Location:clssubmenu.php?report=0&action=insert&message='Insert Fail'");
