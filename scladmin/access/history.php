	...
	<div class="right_div">
		<form action="" enctype="multipart/form-data" method="post">
			<div class="form-group">
			    <input type="text" name="name" class="form-control about_select" placeholder="History Title">
		    </div>
	...
	require_once '../pages/footer.php';
		...
		$storage=basename($newfile);
			...
			if(empty($name) || empty($detail) || empty($image)){
						header("Location:history.php?report=0&action=insert&message='Please Fill All The Field !'");
			}
			else{
				if ($image_type=='image/jpeg' || $image_type=='image/png'){
				$mysql = "INSERT INTO history (ftitle,image,detail) VALUES ('$name','$storage','$detail')";
					move_uploaded_file($img_tmp,"../images/upload/".$storage);
					if($sqlrun = mysql_query($mysql)){
						header("Location:history.php?report=1&action=insert&message=");

					}
					else{
						header("Location:history.php?report=0&action=insert&message='Insert Fail'");
